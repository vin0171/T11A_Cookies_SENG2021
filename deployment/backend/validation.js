...
exports.validateUBL = validateUBL;
const fast_xml_parser_1 = require("fast-xml-parser");
const xmldom_1 = require("xmldom");
const xpath_1 = __importDefault(require("xpath"));
const currencyCode_js_1 = require("../data/currencyCode.js");
function isFormattedYYYYMMDD(str) {
    return /^\d{4}-\d{2}-\d{2}$/.test(str);
}
function selectSingleNode(select, xpathExpr, xmlDoc, label, required = false) {
    const nodes = select(xpathExpr, xmlDoc);
    if (nodes.length > 1) {
...
function validateParty(select, party, label) {
    const nameNodes = select('cbc:Name/text()', party);
    if (nameNodes) {
        const nameValues = nameNodes.map(n => { var _a; return ((_a = n.nodeValue) === null || _a === void 0 ? void 0 : _a.trim()) || ''; });
        const seen = new Set();
...
function validateUBL(invoiceXML) {
    ...
    if (!(fast_xml_parser_1.XMLValidator.validate(invoiceXML) === true)) {
    ...
    try {
        ...
        if (!invoiceId || ((_a = invoiceId.nodeValue) === null || _a === void 0 ? void 0 : _a.trim()) === '') {
        ...
        if (!(issueDate === null || issueDate === void 0 ? void 0 : issueDate.nodeValue) || !isFormattedYYYYMMDD(issueDate.nodeValue)) {
        ...
        if (invoicePeriod) {
            ...
            const hasStart = startDate && ((_b = startDate.nodeValue) === null || _b === void 0 ? void 0 : _b.trim()) !== '';
            const hasEnd = endDate && ((_c = endDate.nodeValue) === null || _c === void 0 ? void 0 : _c.trim()) !== '';
            if (!hasStart && !hasEnd) {
        ...
        const payableAmountElement = selectSingleNode(select, 'cbc:PayableAmount', legalMonetaryTotal, 'Payable Amount', true);
        const currencyID = payableAmountElement.getAttribute('currencyID');
        const payableAmount = payableAmountElement.textContent;
        if (!(payableAmount === null || payableAmount === void 0 ? void 0 : payableAmount.trim())) {
        ...
        if (!(currencyID === null || currencyID === void 0 ? void 0 : currencyID.trim())) {
            throw new Error('Invoice currency is required.');
        }
        if (!currencyCode_js_1.currencyCodes.includes(currencyID)) {
        ...
        for (const invoiceLine of invoiceLines) {
            ...
            const lineExtensionAmount = selectSingleNode(select, 'cbc:LineExtensionAmount', invoiceLine, 'Line Extension Amount', true);
