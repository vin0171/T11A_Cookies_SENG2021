...
describe('getInvoice tests', () => {
    ...
    test('should return invoice details in JSON format successfully', async () => {
      ...
      const invoiceId = invoiceRes.body;
    ...
    test.skip('should return invoice details in UBL format successfully', async () => {
      const adminToken = (await requestUserRegister(app, companyData.companyEmail, globalPassword , "Firstname", "Lastnane")).body;
      const companyId = (await requestCompanyRegister(app, adminToken, companyData)).body;
      const invoiceDetails = { companyId, ...sampleInvoiceDetails };
      const invoiceRes = await requestCreateInvoice(app, adminToken, invoiceDetails);
    ...
    test('should return forbidden error when user is not authorized to view invoice', async () => {
      ...
      const unauthorizedToken = (await requestUserRegister(app,"nonexistentEmail@gmail.com", "globalPassword@123" , "Firstname", "Lastnane")).body;
    ...
    test('should allow a member of the company to view the invoice', async () => {
      ...
      const invoiceRes = await requestCreateInvoice(app, adminToken, invoiceDetails);
      const invoiceId = invoiceRes.body;
      const userToken= (await requestUserRegister(app, 'enemyspotted@gmail.com', globalPassword , "Firstname", "Lastnane")).body;
