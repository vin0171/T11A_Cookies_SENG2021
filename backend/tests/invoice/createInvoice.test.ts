...
describe('createInvoice tests', () => {
    test('Creates an invoice for company from JSON document', async () => {
        ...
        const expectedResponse = expect.any(String)
    ...
    test('Invoice can be created reguardless of whether they in company or not', async () => {
        const adminToken = (await requestUserRegister(app, companyData.companyEmail, globalPassword , "Firstname", "Lastnane")).body;
        const userToken = (await requestUserRegister(app, "testingEmail@gmail.com", globalPassword , "Firstname", "Lastnane")).body;
        const companyId = (await requestCompanyRegister(app, adminToken, companyData)).body;
        // Add the companyId that owns the invoice
        const invoiceDetails = {companyId, ...sampleInvoiceDetails}
        const invoiceRes = (await requestCreateInvoice(app, userToken, invoiceDetails));
        expect(invoiceRes.body).toStrictEqual(expect.any(String));
        expect(invoiceRes.status).toStrictEqual(200);
    });
    ...
    test('Check for unique invoice IDs returned', async () => {
        ...
        invoiceArray.push((await requestCreateInvoice(app, adminToken, invoiceDetails)).body);
        invoiceArray.push((await requestCreateInvoice(app, adminToken, invoiceDetails1)).body);
        invoiceArray.push((await requestCreateInvoice(app, adminToken, invoiceDetails2)).body);
    ...
    //     const companyId = (await requestCompanyRegister(app, companyData)).body;
    //     const adminToken = (await requestUserLogin(app, companyData.companyEmail, companyData.password)).body;
