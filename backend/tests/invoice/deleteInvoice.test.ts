...
describe('deleteInvoice tests', () => {
  ...
  test('Any user can delete invoice regardless of company membership', async () => {
    ...
    // Create second user
    const userToken = (await requestUserRegister(app, "testingEmail@gmail.com", globalPassword, "Firstname", "Lastname")).body;
    
    // Second user attempts to delete first user's invoice
    const deleteRes = await requestDeleteInvoice(app, userToken, createRes.body);
  ...
  test('Deleting multiple invoices from one company', async () => {
    ...
    const delete1 = await requestDeleteInvoice(app, adminToken, invoice1.body);
    const delete2 = await requestDeleteInvoice(app, adminToken, invoice2.body);
    const delete3 = await requestDeleteInvoice(app, adminToken, invoice3.body);
  ...
  test('Error when deleting with invalid token', async () => {
    ...
    const deleteRes = await requestDeleteInvoice(app, 'invalidToken', createRes.body);
