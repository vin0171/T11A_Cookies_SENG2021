...
describe('companyRegister', () => {
  ...
  beforeEach(async () => {
    token = (await requestUserRegister(app, "adminOfCompanyEmail@gmail.com", "adminOfCompanyPw@gmail.com122", "Valid", "Name")).body;
  });
    ...
    test('Sucesfully registers multple company', async () => {
        ...
        const token1 = (await requestUserRegister(app, "info@techcorop2.com", "adminOfCompanyPw@gmail.com122", "Valid", "Name")).body;
        const response2 = await requestCompanyRegister(app, token1, companyData);
        expect(response2.status).toStrictEqual(200);
        expect(response2.body).toStrictEqual(expect.any(String));
        companyData.companyName = 'Turple Tech'
        companyData.companyEmail = 'info@techcorop1.com'
        const token2 = (await requestUserRegister(app, "info@techcorop1.com", "adminOfCompanyPw@gmail.com122", "Valid", "Name")).body;
        const response3 = await requestCompanyRegister(app, token2, companyData);
        expect(response3.body).toStrictEqual(expect.any(String));
        expect(response3.status).toStrictEqual(200);

        const companyId1 = response.body;
